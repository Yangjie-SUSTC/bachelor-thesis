 function ima_str=cell_seg(varargin)
...
if flag==1  
    figlabel(ima_str,'FLUO_lable',path);
    temp=ima_str;
    temp.ima=uint8(BFima);
    imaa=figlabel(temp,'BF_lable',path);
...
function connect=remove_dot(ima)
    ...
    T2=floor(size(ima,1)*size(ima,2)/4000);
